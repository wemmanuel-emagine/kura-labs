...
def forecast(access_key,json_file,csv_file,location):
    ...
    # Inputing the date for the parameter
    date_start = input("Input the start date for the weather history (yy-dd-mm): ")
    date_end = input("Input the end date for the weather history (yy-dd-mm): ")
    ...
    # URL for API
    url_current = "http://api.weatherstack.com/historical"

    # Parameters for the API
    querystring = {"access_key": access_key, "query": location, "historical_date_start": date_start , "historical_date_end": date_end}

    # GET request from API
    response = requests.get( url_current, params=querystring)
    weatherFile = response.text
    inputJsonFile.write(weatherFile)

    # Convert JSON File to CSV File
    # pandas read JSON File
    readWeatherFile = pd.read_json(weatherFile)
    print(readWeatherFile)
